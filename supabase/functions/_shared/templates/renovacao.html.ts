// Template HTML PT-BR para alerta de renovação de contrato.
//
// Placeholders (substituídos por renderRenovacao em render.ts):
//   {{nomeResponsavel}}      — escape HTML
//   {{nomeCliente}}          — escape HTML
//   {{diasAteRenovacao}}     — escape HTML (Number→String)
//   {{valorContrato}}        — escape HTML (string já formatada em BRL pelo caller)
//   {{deepLink}}             — URL literal (não escapar)
//   {{gerenciarPrefsLink}}   — URL literal (não escapar)
//
// Layout consistente com tarefa.html.ts / cadencia.html.ts.

const renovacaoTemplate = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8" />
  <title>Renovação aproximando — CONSEJ</title>
</head>
          ...
          <tr>
            <td style="padding:32px;">
              <h1 style="margin:0 0 16px 0;font-size:20px;line-height:1.4;color:#1a1a1a;">
                Olá, {{nomeResponsavel}} — renovação aproximando
              </h1>
              <p style="margin:0 0 16px 0;font-size:15px;line-height:1.6;color:#3a3a3a;">
                O contrato do cliente <strong>{{nomeCliente}}</strong> vence em <strong>{{diasAteRenovacao}} dia(s)</strong>.
              </p>
              <p style="margin:0 0 24px 0;font-size:14px;line-height:1.6;color:#6a6a6a;">
                Valor do contrato: <strong>{{valorContrato}}</strong>. Hora de iniciar a conversa de renovação.
              </p>
...
export default renovacaoTemplate
